...
function att_show(serverData,file)
{
	...
	if(ext == 1) {
		...
		img = '<div class="get selected" id="aid-'+id+'"><a class="del" href="javascript:;">删除</a> <img onclick="att_cancel(this,'+id+',\'upload\')" width="87" height="98" src="'+GV.DIMAUB+'static/images/ext/'+ext+'.png" data-id="'+id+'" data-path="'+src+'" alt="上传完成" title="'+filename+'"><input type="text" class="J_file_desc" name="flashatt['+id+'][desc]" placeholder="请输入描述" value="'+filename+'"  style="width:68px"></div>';
	}
	//设置附件标识
	$.get(GV.DIMAUB+'index.php?s=Asset/Asset/swfupload_json&aid='+id+'&src='+src+'&filename='+filename);
