...
class AssetController extends \Think\Controller {
	...
	public function swfupload() {
		if (IS_POST) {
			//取得模块名称
			$module = strtolower(I("post.module"));
			...
			$info = $upload->upload();
			if ($info) {
				$img_url = __ROOT__ . '/static/data/upload/' . $info['Filedata']['savepath'] . $info['Filedata']['savename'] . ',1,' . $info['Filedata']['name'];
				//写入附件数据库信息
				echo "1," . $img_url;
				exit;
			} else {
...
//		return service("Attachment")->upload_json($aid, $src, $filename);
	...
	protected function att_not_used() {
		...
		$att_json = \Common\Lib\Util\Input::getVar(cookie('att_json'));
		if ($att_json) {
			if ($att_json) {
